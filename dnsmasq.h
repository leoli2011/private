...
#define OPT_BASE1          OPT_NO_REBIND
#define OPT_ORDER_WEIGHT   (1u<<0)
...
struct server {
  ...
  uint32_t mark;
  char *ipset;
  unsigned int weight, weight_cur, weight_active;
  time_t last_active;
  struct server *next; 
};
...
struct crec *cache_find_by_addr(struct crec *crecp,
				...
				unsigned short prot, struct server **psrv);
struct crec *cache_find_by_name(struct crec *crecp, 
				char *name, time_t now, unsigned short  prot, struct server **psrv);
void cache_end_insert(void);
void cache_start_insert(void);
struct crec *cache_insert(char *name, struct all_addr *addr,
			  time_t now, unsigned long ttl, unsigned short flags, struct server *srv);
...
int extract_addresses(HEADER *header, size_t qlen, char *namebuff, time_t now, struct server *srv);
size_t answer_request(HEADER *header, char *limit, size_t qlen,  
		   struct in_addr local_addr, struct in_addr local_netmask, time_t now, struct server **srv);
int check_for_bogus_wildcard(HEADER *header, size_t qlen, char *name, 
			     struct bogus_addr *addr, time_t now, struct server *srv);
...
/* option.c */
struct server* parse_server_arg(int *option, char *arg, char **problem, int isopt);
...
int add_extend_servers(char *servers);
int del_extend_servers(char *servers);
#endif
void check_servers(void);
void free_server(struct server *serv);
...
void consume_server_weight(struct server *serv, time_t now);
void active_server_weight(struct server *serv);
struct server* find_better_weight_server(time_t now);
